...
class FileSystemStorage extends Storage {
    ...
    async getOrCreateCompilation(requestId) {
        ...
        var compilationId = 'compilation_' + (++this._compilationId);
        var timestamp = Date.now();
        compilation = new Compilation(compilationId, requestId, Compilation.Status.Running, null, timestamp, timestamp);
    ...
    async finishedCompilations() {
        ...
        return compilations.filter(compilation => compilation.status !== Compilation.Status.Running);
    }
