...
class Compilation {
     ...
     */
    constructor(requestId, downloader, resultsFolder) {
    ...
    async _innerRunCompilation() {
        ...
        var {folderPath, target, userError} = await this._downloader.downloadIfNeeded();
        if (!folderPath) {
            this._finishCompilation(false /* success */, userError);
            return;
        }
        var timeout = utils.minutes(5);
        var success = await this._runShellScript(folderPath, target, this.outputPath(), this.logPath(), timeout);
