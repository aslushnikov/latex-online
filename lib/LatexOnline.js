...
class LatexOnline {
    ...
    async compileGit(url, target, branch, forceCompilation) {
        ...
        var method = forceCompilation ? this._storage.createCompilation : this._storage.getOrCreateCompilation;
        var {compilation, writeToken} = await method.call(this._storage, requestId);
